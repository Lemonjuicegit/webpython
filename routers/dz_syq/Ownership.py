...
from . import groupby
class Ownership:
    def __init__(self,gdbpath,precision=10):
        ...
        self.JZD['X'] = np.round(self.JZD.geometry.x*precision).astype('int64')
        self.JZD['Y'] = np.round(self.JZD.geometry.y*precision).astype('int64')
    ...
    def get_coordinates(self,one_gdf):
        ...
        def add_index(x_y,value,V):
            # if V == 87:
            #     pass
            temp = [zddm,*(np.round(x_y*self.precision)).astype('int64'),value,V]
            return temp
    ...
    async def add_jzx_async(self,jzd_gdf_):
        # @jzd_gdf_
    ...
    async def add_jzx_(self):
        ...
        for zddm in zddm_df:
            ...
            for index in coordinates_index:
                sel_jzd = self.JZD[(self.JZD.ZDDM == zddm) & (self.JZD.INDEX == index)].reset_index()
                # pool.apply_async(self.add_jzx,(sel_jzd,))
                self.add_jzx(sel_jzd)
            yield f"正在生成:{zddm}"
    ...
    def to_jzxshp(self,savepath):
        ...
        zd_node['int_X'] = np.round(zd_node.X*self.precision).astype('int64')
        zd_node['int_Y'] = np.round(zd_node.Y*self.precision).astype('int64')
