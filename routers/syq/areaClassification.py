...
def addAreaStatistics(table_,data_={'DKH':'','ZDDM':'','MJ':''}):
    ...
    setCelltext(table_,row_len_,2,str(data_['MJ']))
...
def addZDStatistics(table_,data_={'DKH':'','ZDDM':'','MJ':''}):
    ...
    setCelltext(table_,row_len_,3,data_['MJ'])
...
def collectivelyZDClassifiedArea(data,savePath):
  ...
  if data['fd_data']:
    ...
    setCelltext(table,len(table.rows)-1,3,data['fd_total_area'])
...
def get_data(datapath_,fddata_=''):
  df = pd.read_excel(datapath_)
  datadf = {}
...
# datadf  
def exportArea (datapath,savepath_,df_fda=''):
  data_ = get_data(datapath,df_fda)
  le = len(data_) 
  i = 0     
  for key,value in data_.items():
    i+=1
    savepath = savepath_ / f"{key}集体土地所有权宗地分类面积调查表.docx"
    collectivelyZDClassifiedArea(value,savepath)
    yield f"{savepath}导出成功：{i}/{le}"
